...
- 库：vec_qa_local（float[512]，绑定 bge-small-zh-v1.5，不可混用）。
...
def vector_search(conn, store_id: str, query_vector, top_k: int = TOP_K) -> list:
    ...
    rows = conn.execute(
        ...
        " q.category"
        " FROM vec_qa_local v JOIN qa_pairs q ON q.id = v.qa_id"
