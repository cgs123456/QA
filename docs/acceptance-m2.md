...
## 7. 延期台账（2026-09-15 立；本节是延期项的**唯一权威**）

> 编号说明：本节按派单编号为 §7（§5/§6 未占用）。
> 与上文的关系：§3 是**状态描述**，§4 是 **tag 判定**，本节是**「谁在等谁 + 解除后第一件事」**。
> 纪律：**延期项不得静默转正**；每项必须有**可判定的解除条件**，且解除后第一件事必须写明，
> 否则会退化成「someday」。

### 7.1 M2 四项（全部非「缺代码」，本机不可推进）
...
**G1 的两条路互斥但要分清**：Ollama 路径关的是「本地生成」，云端 key 路径关的是
...
|---|---|---|---|---|
| **`dist-sidecar/` 重建** | 仍是 task-10 旧包（主人已**批准重建**） | 本机批量删除守卫（~50 文件/回合）允许时 | 按 `benchmark.md` 注记的口径重建，并核对与 `sidecar/requirements-lock.txt` 一致 | 2026-09-15 |
...
- **task-14 「错 token → 1008」偏差** → 主人**已接受 403 为准**（2026-09-15），
  PRD 措辞更新挂 PRD 回仓时；`api-contract.md` 已记实测。**不再算延期项。**
