...
> 结论：**14 通过 / 0 未达标（P6.1 翻转 #6，见下）/
> 1 部分通过（#10 LLM 首字待 Ollama）/ 1 待人工走查（#13 降级页渲染）**。
> `m1-text-pipeline` tag 暂缓（见 §4；#6 已达标，暂缓改以后两项为主）。
...
| 6 | 混合检索 100 题 Top-3 >85% | **通过（P6.1 翻转）** | 原未达标证据保留（59 题 Top-3=0.490，零答错，AND 语义根因）。P6 全链标定 + P6.1 补盲区后：冻结集扩至 **124 题（87 scored + 16 null + 21 field）**，`eval_calibrate.py --source` 终值 **Top-3=0.9080（79/87）/ null 16/16 / 答错 machine 15（人工裁决 7，0.088，与初值持平）/ FC(非null) 0.2593（初值 0.3218，改善）**，详见 `docs/eval-final.md` §7。D6 查询理解 + 重标定待办关闭。 |
...
| 阻塞项 | 性质 | 清除条件 | 清除动作 |
|---|---|---|---|
| #6 Top-3 49%（真题） | ~~性能缺口~~ → **已清除（P6.1）**：124 题 Top-3=0.9080，见 §1#6 | D6 查询理解 + 重标定，或扩至 100 题后重估 | 关键词提取 → 重跑基线 → 更新本报告 → 打 tag |
...
| 8 | tag | 不存在：维持暂缓判定（§4；#6 已达标，暂缓改以后两项为主） |
